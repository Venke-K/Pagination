...
//inserting the jason array of objects into the Div element/////
...
function displayItems(items) {
  ...
  items.forEach(item => {
      ...
      itemDiv.innerHTML = `<strong>ID:</strong> ${item.id} <br> <strong>Name:</strong> ${item.name} <br> <strong>Email:</strong> ${item.email}`;
      contentDiv.appendChild(itemDiv);
  });
}


// Using the match.ceil , assinging the the onjects equally///
...
// in this function we are slice the objects start and end //
...
/// setting up the previou and next buttons with disabled option with event listner///
